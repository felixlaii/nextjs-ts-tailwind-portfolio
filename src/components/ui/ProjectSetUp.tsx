    ...
    >
      <div className="flex items-center align-middle">
        <div data-aos="zoom-in">
          <Image
            className="rounded-sm object-center sm:w-[15rem] md:w-[30rem] lg:w-[15rem] w-100"
            width={300}
            height={300}
            src={image}
            alt="project logo"
        ...
        <div className="flex flex-column items-center ml-14 w-86 lg:w-3/4">
          <div className="grid grid-cols-1 gap-1 lg:gap-5 content-around text-center ">
            <div>
              <p
                className={`font-bold text-[0.5rem] sm:text-[0.5rem] sm:tracking-none md:text-[1rem] lg:text-[0.9rem] tracking-wide lg:tracking-widest ${
                  isHovered
            ...
            <div className="flex flex-row justify-evenly">
              {technology.map((iconUrl, index) => (
                <Image
                  ...
                  src={iconUrl}
                  alt={`tech-icon-${index}`}
                  className="lg:w-4 lg:h-5 w-3 h-2 md:w-5 md:h-4 lg:mb-2"
                />
              ))}
            </div>
            ...
            <div>
              <p
                className={`text-[0.4rem] sm:text-[0.4rem] md:text-[0.7rem] lg:text-md font-extralight ${
                  isHovered
                    ? "text-brand-darkMode dark:text-brand-darkest"
                    : "text-brand-darkMode dark:text-brand-darkMode dark:text-brand-darkMode"
                }`}
